...
export function registerExportHandlers(ipcMain, mainWindow) {
  ...
  ipcMain.handle('export:image:markdown', async (_, options = {}) => {
    let imgWindow = null
    let tmpHtmlPath = null
    try {
      const { defaultPath = 'export.png', content, theme, scale = 2 } = options
      ...
      // 写临时 HTML 文件，用 loadFile 加载
      // （data: URL 在内容较长时可能被截断，导致渲染不全/截图空白；临时文件更可靠）
      const tmpDir = app.getPath('temp')
      tmpHtmlPath = join(tmpDir, `mdev-export-${Date.now()}.html`)
      await writeFile(tmpHtmlPath, html, 'utf8')

      // 窗口宽度固定（对应 CSS max-width:900px + padding），高度按内容动态调整
      const baseWidth = 940
      ...
      const pngBuffer = await new Promise((resolve, reject) => {
        ...
        imgWindow.webContents.on('did-finish-load', async () => {
          try {
            // 等待 KaTeX 字体（woff2 inline 在 CSS 里）和图片等异步资源加载
            await new Promise(r => setTimeout(r, 800))

            // 测量内容实际尺寸（原始 CSS 像素）
            // 不使用 setZoomFactor —— 它会让内容放大，导致 scrollWidth/scrollHeight
            // 与 setContentSize 的尺寸计算混乱，最终截图被裁剪或空白
            const dims = await imgWindow.webContents.executeJavaScript(
              `JSON.stringify({
            ...
            const targetWidth = Math.max(sw, baseWidth)
            // 限制最大高度避免内存溢出
            const targetHeight = Math.min(sh + 40, 30000)
            imgWindow.setContentSize(targetWidth, targetHeight)

            // 等待重排和字体重新渲染稳定
            await new Promise(r => setTimeout(r, 500))

            // 截取整个窗口客户区
            const img = await imgWindow.webContents.capturePage()
            if (img.isEmpty()) {
              finish(reject, new Error('截图返回空图像，可能内容未渲染完成'))
              return
            }

            // 高清缩放：用 nativeImage.resize 放大
            // （比 setZoomFactor + setContentSize 组合更可靠，参考 puppeteer page.screenshot 做法：
            //  先原始尺寸截取完整内容，再按需放大。resize 用 best 质量插值）
            let finalImg = img
            if (scale !== 1) {
              const origSize = img.getSize()
              finalImg = img.resize({
                width: Math.round(origSize.width * scale),
                height: Math.round(origSize.height * scale),
                quality: 'best'
              })
            }

            finish(resolve, finalImg.toPNG())
          } catch (err) {
            finish(reject, err)
          }
        })

        imgWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription) => {
          finish(reject, new Error(`页面加载失败: ${errorDescription} (code ${errorCode})`))
        })

        imgWindow.loadFile(tmpHtmlPath)
      })
      ...
      // 确保窗口和临时文件被清理
      if (imgWindow && !imgWindow.isDestroyed()) {
        imgWindow.close()
      }
      if (tmpHtmlPath) {
        try { await unlink(tmpHtmlPath) } catch {}
      }
    }
  })
